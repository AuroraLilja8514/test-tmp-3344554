...
This repository is currently in **Phase 0: feasibility probes**. The first probe established that a child-only `TZ` environment variable reaches the launched process but is **not sufficient for Chromium on Windows**. The corrected second probe has now proven a Chromium-native override installed before normal Electron application startup.
...
## Confirmed Phase 0B result: early Electron renderer override

Electron exposes a main-process Node inspector through `--inspect-brk`. The corrected Phase 0B probe pauses the Electron main JavaScript before normal app code runs, attaches only through a loopback inspector port, and installs an in-memory `web-contents-created` listener by evaluating on the paused call frame with `Debugger.evaluateOnCallFrame`. After Electron resumes, each WebContents receives Chromium's native:

`Emulation.setTimezoneOverride`

Tested against Codex desktop `26.825.6671.0`, the renderer reported:
...
```text
CHROMIUM_OVERRIDE_PASS: renderer reports 'Pacific/Honolulu'
```
...
### Safe prerequisite preflight

Before `ChatGPT.exe` is created, `launch-codex-early.ps1` verifies all prerequisites that could otherwise strand Codex at `--inspect-brk`:

- the requested Python executable resolves successfully;
- Python is 3.10 or newer;
- `websocket-client` imports successfully and exposes `websocket.create_connection`;
- `bootstrap-electron-timezone-callframe.py` exists;
- the helper compiles as valid Python.

If any check fails, the script exits **without starting Codex**. This includes an invalid `-Python` path and a missing `websocket-client` installation.

After Codex is launched, the helper also attempts to resume Electron in its failure path if it has connected to the inspector, so a diagnostic failure does not intentionally leave Codex paused.
...
On Windows those libraries use Windows timezone facilities rather than the Chromium renderer override. Consequently, the Phase 0B PASS proves the Chromium/UI half only. Phase 0C is testing a process-local Windows timezone API shim for the native `codex.exe app-server` side.

There is also a third surface: shell/tool processes spawned by Codex. Codex normally derives their environment from the app-server process environment according to its shell environment policy. The default policy inherits the full environment, so the launcher-provided child-only `TZ` can reach tool processes; however, individual Windows runtimes do not all interpret `TZ` the same way. The final design must therefore keep Chromium, native Windows time APIs, and `TZ`-aware child runtimes consistent.
...
After the native-process mechanism is proven, the production application will be Windows-only C#/.NET (WinForms) with:
